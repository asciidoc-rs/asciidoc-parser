...
track_file!("docs/modules/macros/pages/icons-font.adoc");
...
"#
    ...
    // Setting `icons` to `font` selects an icon-font glyph (`<i class="fa …">`)
    // for the icon macro, influencing only the converted output.
    ...
    // The attribute has no affect on parsing: the source structure is unchanged
    // relative to a document without it (only the rendered output differs).
    let doc = Parser::default().parse("icon:heart[]");
    let block = doc.nested_blocks().next().unwrap();
    assert_eq!(block.raw_context().as_ref(), "paragraph");
}

// Everything below describes converter, stylesheet, and CSS behavior (font
// selection, recommended admonition-icon mappings, and callout styling) that is
// out of scope for this parser crate: it influences only the converted output,
