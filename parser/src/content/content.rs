...
mod tests {
    ...
    mod assign_footnote_tree_xrefs {
        use super::super::assign_footnote_tree_xrefs;
        use crate::{
            Span,
            inlines::{Footnote, InlineNode, Ref, RefVariant, SpanForm, StyleVariant, Styled},
            parser::ResolvedReference,
        };

        /// A cross-reference node, unresolved.
        fn xref() -> InlineNode<'static> {
            InlineNode::Ref(Ref {
        ...
        /// A footnote node whose subtree holds one cross-reference.
        fn footnote_over(child: InlineNode<'static>) -> InlineNode<'static> {
            InlineNode::Footnote(Footnote {
                id: None,
                number: Some("1".into()),
                is_reference: false,
        ...
        /// The resolved `href` of the first cross-reference anywhere in
        /// `nodes`.
        fn first_href(nodes: &[InlineNode<'_>]) -> Option<String> {
            for node in nodes {
                let found = match node {
                    InlineNode::Ref(reference) => reference
                        ...
                        .or_else(|| first_href(&reference.children)),

                    InlineNode::Styled(styled) => first_href(&styled.children),
                    InlineNode::Footnote(footnote) => first_href(&footnote.children),
                    _ => None,
                };
        ...
        fn destinations() -> Vec<Option<ResolvedReference>> {
            vec![Some(ResolvedReference::new("#tgt".to_string(), None))]
        }

        #[test]
        fn reaches_a_footnote_nested_in_a_reference() {
            // A footnote written inside a link's display text is a `Footnote`
            // node under a `Ref` node, so the walk descends through reference
            // children to reach its subtree.
            let mut nodes = vec![InlineNode::Ref(Ref {
                variant: RefVariant::Link,
                target: "https://example.org".into(),
                children: vec![footnote_over(xref())],
                roles: vec![],
                window: None,
                resolved: None,
                location: Span::new(""),
            ...
            let mut next = 0;
            assign_footnote_tree_xrefs(&mut nodes, &destinations(), &mut next);

            assert_eq!(next, 1);
            assert_eq!(first_href(&nodes).as_deref(), Some("#tgt"));
        }

        #[test]
        fn reaches_a_footnote_nested_in_a_formatting_span() {
            let mut nodes = vec![InlineNode::Styled(Styled {
        ...
        #[test]
        fn leaves_a_block_level_reference_untouched() {
            // Only footnote subtrees are the footnote walk's business; a
            // block-level cross-reference is the other walk's, and must not
            // consume a footnote slot.
            let mut nodes = vec![xref()];

            let mut next = 0;
            assign_footnote_tree_xrefs(&mut nodes, &destinations(), &mut next);

            assert_eq!(next, 0);
            assert_eq!(first_href(&nodes), None);
        }
    }
}
