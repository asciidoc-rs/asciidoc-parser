...
Added::
* Add examples for how to avoid first line of paragraph from being matched as a block attribute line

== Published Q1 2026

Added::
* Document the `window` attribute of the xref macro

Changed::
* Clarify that `indent` attribute on verbatim block ignored if at least one line is not indented
* Clarify that the preface for a multipart book may be defined as either a level 0 or level 1 section
...
* Add example of multiple roles assigned to formatted text

Changed::
* Document that the toclevels value ranges from 0-5 rather than 1-5 and that 0 is only relevant for a multipart book
...
* Clarify the typical order for shorthand attributes in a boxed attrlist
